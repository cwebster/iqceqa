class ImportedFileController < ApplicationController
  # GET /importedfile
  # GET /importedfile.json
  def index
    @imported_files = ImportedFile.all
    @no_records_by_date = ImportedFile.count("DATE(created_at)")
    @imported_files_by_date = ImportedFile.group("DATE(created_at)").count
    
    StatHat::API.ez_post_value("AMS Files Imported", "craig.webster@heartofengland.nhs.uk", @no_records_by_date)
